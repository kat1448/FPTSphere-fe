...
/**
 * Get events pending approval (for Event Manager dashboard)
 *
 * API Endpoint: GET /Events/pending-approval
 *
 * Response format:
 * {
 *   "success": true,
 *   "message": "Retrieved N events pending approval",
 *   "data": [ ... ]
 * }
 *
 * @returns {Promise<Array>} Array of pending approval events
 */
export const getPendingApprovalEvents = async () => {
  try {
    console.log("📤 Fetching events pending approval");

    const response = await apiClient.get(`${EVENTS_ENDPOINT}/pending-approval`, {
      headers: { Accept: "*/*" },
    });
    ...
    console.error("❌ Get pending approval events error:", error);
    const errorMessage =
      error.response?.data?.message || error.message || "Failed to fetch pending approval events";
    throw new Error(errorMessage);
  }
};
 ...
 *
 * API Endpoint: PUT /Events/{id}
 *
 * Swagger expects multipart/form-data with fields:
 * EventName (required), Description, BannerUrl(file), StartTime, EndTime,
 * ExpectedAttendees, EstimatedCost, LocationId, ExternalLocationId, TemplateId, CategoryId, TypeId
 *
...
export const updateEvent = async (eventId, data = {}) => {
  try {
    ...
    fd.append("EventName", String(data.eventName).trim());

    if (data.description !== undefined) {
      const d = data.description === null ? "" : String(data.description);
      if (d.trim() !== "") fd.append("Description", d);
    }

    // BannerUrl is IFormFile (optional)
    if (data.bannerFile) {
      fd.append("BannerUrl", data.bannerFile);
    }

    // Datetimes must be ISO 8601 (Swagger shows Z)
    fd.append("StartTime", typeof data.startTime === "string" ? data.startTime : data.startTime.toISOString());
    fd.append("EndTime", typeof data.endTime === "string" ? data.endTime : data.endTime.toISOString());
    ...
    appendNumber("ExpectedAttendees", data.expectedAttendees);
    appendNumber("EstimatedCost", data.estimatedCost);
    appendNumber("LocationId", data.locationId);
    appendNumber("ExternalLocationId", data.externalLocationId);
    ...
    const response = await apiClient.put(`${EVENTS_ENDPOINT}/${eventId}`, fd, {
      headers: {
        Accept: "*/*",
        // don't set Content-Type; browser will set boundary for FormData
      },
    });
...
/**
 * Update sub-event (multipart/form-data)
 *
 * API Endpoint: PUT /Events/subevents/{subEventId}
 *
 * Swagger expects multipart/form-data with fields:
 * EventName (required), Description, BannerUrl(file), StartTime (required), EndTime (required),
 * ExpectedAttendees, EstimatedCost, LocationId, ExternalLocationId, CategoryId, TypeId
 *
 * @param {number} subEventId
 * @param {Object} data
 * @returns {Promise<Object>}
 */
export const updateSubEvent = async (subEventId, data = {}) => {
  try {
    if (!subEventId) throw new Error("Sub-event ID is required");
    if (!data.eventName || String(data.eventName).trim() === "") {
      throw new Error("Event name is required");
    }
    if (!data.startTime || !data.endTime) {
    ...
    const response = await apiClient.put(`${EVENTS_ENDPOINT}/subevents/${subEventId}`, fd, {
      headers: {
        Accept: "*/*",
        // don't set Content-Type; browser will set boundary for FormData
      },
    ...
    console.error(`❌ Update sub-event error (ID: ${subEventId}):`, error);
    const errorMessage = error.response?.data?.message || error.message || "Failed to update sub-event";
    throw new Error(errorMessage);
  }
};
...
export const deleteEvent = async (eventId) => {
  try {
    ...
    const response = await apiClient.delete(`${EVENTS_ENDPOINT}/${eventId}`, {
      headers: { Accept: "*/*" },
    });
    ...
    console.error(`❌ Delete event error (ID: ${eventId}):`, error);
...
/**
 * Reject event (main event or sub-event)
 *
 * API Endpoint: POST /Events/{eventId}/reject
 *
 * @param {number} eventId - Event ID (can be main event or sub-event)
 * @param {string} comment - Rejection comment
 * @returns {Promise<Object>}
 */
export const rejectEvent = async (eventId, comment = "") => {
  try {
    if (!eventId) {
      throw new Error("Event ID is required");
    }

    console.log(`📤 Rejecting event ID: ${eventId} with comment: "${comment}"`);

    const response = await apiClient.post(
      `${EVENTS_ENDPOINT}/${eventId}/reject`,
      {
        comment: comment || "",
      },
      {
    ...
    // Status 200 (OK) or 201 (Created): Success
    if (response.status === 200 || response.status === 201) {
      console.log("✅ Event rejected successfully:", response.data);
    ...
    console.error(`❌ Reject event error (Event ID: ${eventId}):`, error);

    // Handle validation errors
    if (error.response?.status === 400) {
      const errorData = error.response.data;
      const errorMessage = errorData?.message || "Cannot reject this event";
      const formattedError = new Error(errorMessage);
      formattedError.status = 400;
      formattedError.data = errorData;
      throw formattedError;
