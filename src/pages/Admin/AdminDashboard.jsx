...
import { Input, Select, Table, Avatar, Tag, Progress, Button, Card, Statistic, Row, Col, Modal, message } from "antd";
import { CheckCircleOutlined as ApproveIcon } from "@ant-design/icons";
import dayjs from "dayjs";
import { getEvents, approveEvent } from "../../services/events.api";
...
const AdminDashboard = () => {
  ...
  const isDirector = roleId === 2 || roleName === "Director";
  ...
  const handleApprove = async () => {
    ...
    try {
      setApproving(true);
      console.log(`📤 Approving event ${selectedEvent.eventId} with comment: "${approveComment}"`);

      await approveEvent(selectedEvent.eventId, approveComment);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 150,
      render: (_, record) => {
        const canApprove = isDirector && record.statusId === 1; // Only Director can approve Pending events (statusId = 1)
        ...
        return (
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => {
                if (isDirector) {
                  // Director: Navigate to approve/view event page
                  navigate(`/admin/events/${record.eventId || record.id}/approve`);
                } else {
                  // Admin: Navigate to sub-events
                  navigate(`/admin/events/${record.eventId || record.id}/sub-events`);
                }
              }}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            {roleName} Dashboard • Manage events, users, and system settings
          </p>
        </div>
        ...
        <Modal
          ...
          okText="Approve"
          okButtonProps={{ type: "primary", icon: <ApproveIcon /> }}
        >
          {selectedEvent && (
            <div>
              <p className="mb-4">
