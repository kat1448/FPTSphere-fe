...
import { getParticipants } from "../../services/participants.api";
...
const Reports = () => {
  ...
  const [participantsData, setParticipantsData] = useState(new Map()); // Map<`${eventId}-${subEventId}`, {totalCount, ...}>

  // Load all events and participants data
  const loadEvents = async () => {
    try {
      ...
      if (response && response.data) {
        ...
        // Load participants data for all events and sub-events
        await loadParticipantsData(response.data);
      }
    } catch (error) {
  ...
  // Load participants data for all events and sub-events
  const loadParticipantsData = async (eventsList) => {
    const participantsMap = new Map();
    
    // Get all main events and sub-events
    const allEvents = eventsList.filter(e => e.parentEventId === null);
    const allSubEvents = eventsList.filter(e => e.parentEventId !== null);
    
    // Load participants for main events (try with subEventId = 0)
    for (const event of allEvents) {
      try {
        // For main events, try to get participants with subEventId = 0
        const participantsResponse = await getParticipants(event.eventId, 0);
        if (participantsResponse?.totalCount !== undefined) {
        ...
        // If no participants found, that's okay - try with the event's own ID as subEventId
        try {
          const participantsResponse = await getParticipants(event.eventId, event.eventId);
          if (participantsResponse?.totalCount !== undefined) {
            const key = `${event.eventId}-${event.eventId}`;
            participantsMap.set(key, {
              totalCount: participantsResponse.totalCount || 0,
              checkedInCount: participantsResponse.checkedInCount || 0,
              checkedOutCount: participantsResponse.checkedOutCount || 0,
          ...
          // If still no participants found, that's okay
          console.log(`No participants for main event ${event.eventId}`);
        }
      }
    }
    
    // Load participants for sub-events
    for (const subEvent of allSubEvents) {
      try {
        const participantsResponse = await getParticipants(subEvent.parentEventId, subEvent.eventId);
        if (participantsResponse?.totalCount !== undefined) {
          const key = `${subEvent.parentEventId}-${subEvent.eventId}`;
          participantsMap.set(key, {
            totalCount: participantsResponse.totalCount || 0,
            checkedInCount: participantsResponse.checkedInCount || 0,
            checkedOutCount: participantsResponse.checkedOutCount || 0,
        ...
        // If no participants found, that's okay
        console.log(`No participants for sub-event ${subEvent.eventId}`);
      }
    }
  ...
  const statistics = useMemo(() => {
    ...
    // Calculate actual participant attendance from API data
    let totalParticipants = 0;
    let totalRevenue = 0;
    
    organizedEvents.forEach((event) => {
      // Get participants for main event (try both 0 and eventId as subEventId)
      const mainEventKey1 = `${event.eventId}-0`;
      const mainEventKey2 = `${event.eventId}-${event.eventId}`;
      const mainEventParticipants = participantsData.get(mainEventKey1) || participantsData.get(mainEventKey2);
      if (mainEventParticipants) {
        totalParticipants += mainEventParticipants.totalCount || 0;
      }
      
      // Get participants for sub-events
      event.subEvents.forEach((subEvent) => {
        const subEventKey = `${event.eventId}-${subEvent.eventId}`;
        const subEventParticipants = participantsData.get(subEventKey);
        if (subEventParticipants) {
          totalParticipants += subEventParticipants.totalCount || 0;
        }
      });
      
      // Calculate revenue if event has actualRevenue field
      // Otherwise, revenue is 0 (or could be calculated from participants × ticket price)
      if (event.actualRevenue !== undefined && event.actualRevenue !== null) {
        totalRevenue += Number(event.actualRevenue) || 0;
      }
      
      // Also check sub-events for revenue
      event.subEvents.forEach((subEvent) => {
        if (subEvent.actualRevenue !== undefined && subEvent.actualRevenue !== null) {
          totalRevenue += Number(subEvent.actualRevenue) || 0;
        }
      });
    });
  ...
  }, [organizedEvents, participantsData]);
  ...
  const exportToExcel = () => {
    try {
      ...
      const summaryData = [
        ...
        ["Total Participants (Actual)", statistics.totalParticipants],
      ];
