...
const ManagerDashboard = () => {
  ...
  // Refresh events when component becomes visible (e.g., after navigation from create event)
  useEffect(() => {
    const handleFocus = () => {
      if (userId) {
        fetchEvents();
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className="flex items-center gap-2">
          ...
          <Button
            type="link"
            onClick={() => navigate(`/event-manager/events/${record.eventId || record.id}/assign-tasks`)}
            className="p-0 text-[14px]"
            style={{ color: "#F2721E" }}
          >
            Assign Tasks
