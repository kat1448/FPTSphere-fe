...
import { createLocation, getLocations } from "../../../../services/locations.api.js";
...
export default function SelectRoomModal({
  ...
  useEffect(() => {
    if (open && locationMode === "offline" && offlineType === "internal") {
      loadRooms();
    }
  }, [open, locationMode, offlineType]);
  ...
  const loadRooms = async () => {
    ...
    try {
      // API: GET /Locations?page=1&pageSize=10&sortBy=Name&sortDescending=false
      ...
      locations = locations.filter(loc => loc.isActive !== false);
      ...
      if (e.response?.status) {
        const errorMsg = e.response?.data?.message || "Failed to load locations";
