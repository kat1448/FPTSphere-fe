...
import { getSubEvents, getEventById } from "../../../../services/events.api.js";
...
export default function Step3Resources({ onPrev, onNext }) {
  ...
  // Load sub-events from API or session storage
  const loadSubEvents = async (eventId) => {
    ...
    try {
      ...
      // Fetch detailed location info for each sub-event if needed
      // We'll fetch in parallel but with error handling
      const enrichedSubEvents = await Promise.allSettled(
        subEventsList.map(async (sub, index) => {
          let locationDetails = null;
          
          // If we have locationId, try to fetch detailed location info
          if (sub.locationId) {
            try {
              const detailedEvent = await getEventById(sub.eventId);
              if (detailedEvent?.location) {
                locationDetails = {
                  locationId: detailedEvent.location.locationId,
                  name: detailedEvent.location.name,
                  building: detailedEvent.location.building,
                  roomNumber: detailedEvent.location.roomNumber,
                  capacity: detailedEvent.location.capacity,
                  locationMode: "internal",
                };
              } else {
                // Fallback to basic location info from sub-events list
                locationDetails = {
              ...
              console.warn(`Failed to fetch detailed location for sub-event ${sub.eventId}:`, error);
              // Fallback to basic location info from sub-events list
              locationDetails = {
                locationId: sub.locationId,
                name: sub.locationName,
          ...
          } else if (sub.externalLocationId) {
            locationDetails = {
          ...
          return {
            ...
            locationMode: locationDetails?.locationMode || (sub.locationId ? "internal" : sub.externalLocationId ? "external" : null),
            locationId: sub.locationId,
            externalLocationId: sub.externalLocationId,
            locationName: sub.locationName, // Keep for fallback display
            externalLocationName: sub.externalLocationName,
            bannerUrl: sub.bannerUrl || FALLBACK_BANNERS[index % FALLBACK_BANNERS.length],
            expectedAttendees: sub.expectedAttendees ?? 0,
            estimatedCost: sub.estimatedCost ?? 0, // Use estimatedCost from API
            feasibility: sub.feasibility || "High",
            quotationItems: sub.quotationItems || [],
            totalCost: sub.estimatedCost ?? sub.totalCost ?? 0, // Prefer estimatedCost from API
      ...
      const successfulSubEvents = enrichedSubEvents
        .map((result, index) => {
          if (result.status === "fulfilled") {
            return result.value;
          } else {
            console.error(`Failed to process sub-event at index ${index}:`, result.reason);
            // Fallback: use basic data from subEventsList
            const sub = subEventsList[index];
            return {
              key: index,
              eventId: sub.eventId,
              eventName: sub.eventName,
      ...
      setRows(successfulSubEvents);
      // Also update session storage for consistency
      WizardSS.set("subEvents", successfulSubEvents);
    } catch (error) {
      console.error("Error loading sub-events:", error);
      message.error("Failed to load sub-events. Using cached data.");
  ...
  useEffect(() => {
    ...
    // Load sub-events from API if mainEvent has eventId, otherwise from session storage
    if (me.eventId) {
      loadSubEvents(me.eventId);
    } else {
      // Fallback to session storage if no eventId (new event being created)
      const subs = WizardSS.get("subEvents", []);
    const mapped = subs.map((sub, index) => ({
      key: index,
      ...sub,
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 220,
      render: (_, record) => {
        const locationName = record.selectedLocation?.name || 
                             record.locationName || 
                             record.selectedLocation?.code || 
                             "TBA";
        const building = record.selectedLocation?.building;
        const roomNumber = record.selectedLocation?.roomNumber;
        const locationMode = record.locationMode || 
                            (record.locationId ? "internal" : 
                             record.externalLocationId ? "external" : 
                             record.selectedLocation?.locationMode);
