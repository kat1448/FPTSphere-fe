...
// Helper function to get current time in GMT+7 (Vietnam timezone)
// GMT+7 is UTC+7, so we add 7 hours to UTC time
const getNowGMT7 = () => {
  const now = new Date();
  // Get UTC timestamp
...
// Helper function to convert dayjs object to GMT+7 for comparison
...
export default function Step1MainEvent({ onNext }) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const startGMT7 = toGMT7(values.startTime);
      const endGMT7 = toGMT7(values.endTime);

      // Validate: Start date must be after current time in GMT+7
      if (startGMT7.isBefore(nowGMT7) || startGMT7.isSame(nowGMT7)) {
        return setErr(`Start date must be after current time (${nowGMT7.format("YYYY-MM-DD HH:mm")} GMT+7)`);
      }

      // Validate: End date must be after start date
      if (endGMT7.isBefore(startGMT7) || endGMT7.isSame(startGMT7)) {
        return setErr(`End date must be after start date (${startGMT7.format("YYYY-MM-DD HH:mm")} GMT+7)`);
      }
        ...
        <div className="form-section" style={{ marginBottom: "12px" }}>
          ...
          >
            <Upload
              ...
              onPreview={(file) => {
                // Show preview modal when clicking on image
                ...
                rules={[
                  { required: true, message: "Please select start date" },
                  {
                    validator: (_, value) => {
                      ...
                      // Convert selected value to GMT+7 for comparison
                      const selectedGMT7 = toGMT7(value);
                      
                      // Validate: Start date must be after current time in GMT+7
                      if (selectedGMT7.isBefore(nowGMT7) || selectedGMT7.isSame(nowGMT7)) {
                        return Promise.reject(
                          new Error(`Start date must be after current time (${nowGMT7.format("YYYY-MM-DD HH:mm")} GMT+7)`)
                        );
                      }
              ...
              >
                <DatePicker
                  ...
                  disabledDate={(current) => {
                    // Disable dates before today in GMT+7
                    const nowGMT7 = getNowGMT7();
                    return current && current.isBefore(nowGMT7, 'day');
                  }}
                  disabledTime={(current) => {
                    // If selected date is today, disable past hours
                    const nowGMT7 = getNowGMT7();
                    if (current && current.isSame(nowGMT7, 'day')) {
                      return {
                        disabledHours: () => {
                          const hours = [];
                          for (let i = 0; i < nowGMT7.hour(); i++) {
                            hours.push(i);
                          }
                          return hours;
                        },
                        disabledMinutes: (selectedHour) => {
                          if (selectedHour === nowGMT7.hour()) {
                            const minutes = [];
                            for (let i = 0; i <= nowGMT7.minute(); i++) {
                              minutes.push(i);
                            }
                            return minutes;
                          }
                ...
                rules={[
                  ...
                  ({ getFieldValue }) => ({
                    validator: (_, value) => {
                      ...
                      // Convert to GMT+7 for comparison
                      const startGMT7 = toGMT7(startTime);
                      const endGMT7 = toGMT7(value);
                      
                      // Validate: End date must be after start date
                      if (endGMT7.isBefore(startGMT7) || endGMT7.isSame(startGMT7)) {
                        return Promise.reject(
                          new Error(`End date must be after start date (${startGMT7.format("YYYY-MM-DD HH:mm")} GMT+7)`)
                        );
                      }
              ...
              >
                <DatePicker
                  ...
                  disabledDate={(current) => {
                    ...
                    if (!startTime) {
                      // If no start time selected, disable dates before today
                      const nowGMT7 = getNowGMT7();
                      return current && current.isBefore(nowGMT7, 'day');
                    }
                    // Disable dates before start date
                    const startGMT7 = toGMT7(startTime);
                    return current && current.isBefore(startGMT7, 'day');
                  }}
                  disabledTime={(current) => {
                    const startTime = form.getFieldValue('startTime');
                    if (!startTime) {
                    ...
                    const startGMT7 = toGMT7(startTime);
                    const currentGMT7 = toGMT7(current);
                    
                    // If selected date is same as start date, disable hours/minutes before start time
                    if (currentGMT7 && startGMT7 && currentGMT7.isSame(startGMT7, 'day')) {
                      return {
                        disabledHours: () => {
                          const hours = [];
                          for (let i = 0; i < startGMT7.hour(); i++) {
                            hours.push(i);
                          }
                          return hours;
                        },
                        disabledMinutes: (selectedHour) => {
                          if (selectedHour === startGMT7.hour()) {
                            const minutes = [];
                            for (let i = 0; i <= startGMT7.minute(); i++) {
                              minutes.push(i);
                            }
                            return minutes;
                          }
